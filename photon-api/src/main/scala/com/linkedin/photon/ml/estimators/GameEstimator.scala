...
class GameEstimator(val sc: SparkContext, implicit val logger: Logger) extends PhotonParams {
  ...
  val featureShardStatistics: Param[Map[FeatureShardId, BasicStatisticalSummary]] = ParamUtils.createParam[Map[FeatureShardId, BasicStatisticalSummary]](
    "the global population feature statistics summary",
  ...
  def setFeatureShardStatistics(value: Map[FeatureShardId, BasicStatisticalSummary]) : this.type = set(featureShardStatistics, value)
