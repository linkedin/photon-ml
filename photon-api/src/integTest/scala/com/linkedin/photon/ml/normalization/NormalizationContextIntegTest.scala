...
class NormalizationContextIntegTest extends SparkTestUtils with GameTestUtils {
  ...
  /**
   * This is a sophisticated test for standardization with the heart data set. An objective function with normal input
   * and a loss function with standardization context should produce the same result as an objective function with
   * standardized input and a plain loss function. The heart data set seems to be well-behaved, so the final objective
