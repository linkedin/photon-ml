...
class SystemToPhysicalReportTransformer
  ...
  def transform(sysrep: SystemReport): ChapterPhysicalReport = {
    sysrep match {
//      case SystemReport(nameIdx, _) =>
//        throw new IllegalStateException("System report should, at least, include non-null command line parameters")
      case SystemReport(nameIdx, None) =>
        new ChapterPhysicalReport(
          Seq(PARAMETERS_TRANSFORMER.transform(ParametersReport())), SYSTEM_CHAPTER_HEADER)
      case SystemReport(nameIdx, Some(summary)) =>
        new ChapterPhysicalReport(
          Seq(
            PARAMETERS_TRANSFORMER.transform(ParametersReport()),
