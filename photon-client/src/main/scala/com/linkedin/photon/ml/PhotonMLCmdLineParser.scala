...
object PhotonMLCmdLineParser {
   ...
   * ]
   *
   * @note In addition to being called here, this function is also called from the Photon templates library to
  ...
  /**
   * Parse [[Params]] from the command line using [[http://scopt.github.io/scopt/3.3.0/api/#scopt.OptionParser scopt]].
   *
  ...
  def parseFromCommandLine(args: Array[String]): Params = {
    ...
    val parser = new OptionParser[Unit]("Photon-ML") {
      ...
      opt[String](REGULARIZATION_WEIGHTS_OPTION)
        .text("Comma separated list of distinct regularization weights. The regularization weight will be ignored if " +
            s"$REGULARIZATION_TYPE_OPTION is set to ${RegularizationType.NONE}. " +
      ...
      opt[Boolean](KRYO_OPTION)
        .text("Whether to use kryo to serialize the intermediate result, which is generally a good idea. More " +
            "information on serialization method used in Spark can be found through the following link: " +
            s"https://spark.apache.org/docs/latest/tuning.html#data-serialization. Default: ${defaultParams.kryo}")
      ...
      opt[Int](MIN_NUM_PARTITIONS_OPTION)
        .text(s"The minimum number of Hadoop splits to generate.. Default: ${defaultParams.minNumPartitions}. " +
            "This would be potentially helpful when the number of default Hadoop splits is small. Note " +
            "that when the default number of Hadoop splits (from HDFS) is larger than min-partitions, " +
            "then min-partitions will be ignored and the number of partitions of the resulting RDD will " +
            "be same as the default number of Hadoop splits. In short, min-partitions will *only* be able " +
            "to increase the number of partitions.")
        .foreach(x => params.minNumPartitions = x)

      opt[Boolean](VALIDATE_PER_ITERATION)
        .text("If validating data is provided, and optimization tracker is enabled, whether to compute the " +
            "evaluation metrics on validating data per iteration")
      ...
      opt[String](COEFFICIENT_BOX_CONSTRAINTS)
        .text("JSON array of maps specifying bound constraints on coefficients. The input is expected to be an array " +
            s"of maps with the map containing keys only from {${ConstraintMapKeys.values.mkString(s",")}} " +
            "(other keys will be ignored")
        .validate(x =>
          if (checkConstraintStringValidity(x)) {
            success
          } else {
            failure(s"Could not parse the input constraint string [$x]. The input is expected to be an array of maps " +
                s"with the map containing keys only from {${ConstraintMapKeys.values.mkString(",")}}. An example " +
