...
/**
 * Simple test for GAME training's [[GameTrainingParams]].
 */
class GameTrainingParamsTest {

  import GameTrainingParams._
  import GameTrainingParamsTest._

  def parse(args: Map[String, String]): GameTrainingParams = GameTrainingParams.parseFromCommandLine(mapToArray(args))
  ...
  def testFeatureShardIdToFeatureSectionKeysMap(): Unit = {
    ...
    val params =
      GameTrainingParams.parseFromCommandLine(mapToArray(requiredArgsModified(FEATURE_SHARD_ID_TO_FEATURE_SECTION_KEYS_MAP,
        argValueInStr)))
  ...
  def testRandomEffectOptimizationConfigurations(): Unit = {
    ...
    val params = GameTrainingParams.parseFromCommandLine(mapToArray(requiredArgsModified(RANDOM_EFFECT_OPTIMIZATION_CONFIGURATIONS,
      argValueInStr)))
    val config1 = GLMOptimizationConfiguration.parseAndBuildFromString(config1InStr)
    val config2 = GLMOptimizationConfiguration.parseAndBuildFromString(config2InStr)
    val expectedValue = Array(
  ...
  def testFactoredRandomEffectOptimizationConfigurations(): Unit = {
    ...
    val params =
      GameTrainingParams.parseFromCommandLine(mapToArray(requiredArgsModified(FACTORED_RANDOM_EFFECT_OPTIMIZATION_CONFIGURATIONS,
        argValueInStr)))
