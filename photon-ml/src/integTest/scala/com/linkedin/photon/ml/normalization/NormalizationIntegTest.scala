...
class NormalizationIntegTest extends SparkTestUtils {
  ...
  private def generateSampleRDD(seed: Int, model: LogisticRegressionModel): RDD[LabeledPoint] = {
    val data = drawBalancedSampleFromNumericallyBenignDenseFeaturesForBinaryClassifierLocal(seed, _size, _dimension).map {
      case (_, sparseVector: SparseVector[Double]) =>
        ...
        val label = model.predict(vector, _threshold)
  ...
  /**
   * Check the correctness of training with a specific normalization scheme. This check involves an unregularized training. The prediction of an unregularized
   * trained model should predict exactly the same label as the one in the input training data, given that the labels are generated by a model without noise.
   ...
   */
  private def checkTrainingOfNormalizationType(trainRDD: RDD[LabeledPoint], testRDD: RDD[LabeledPoint], normalizationType: NormalizationType): Unit = {
    ...
    // For all types of normalization, the unregularized trained model should predict the same label.
    trainRDD.foreach {
      case LabeledPoint(label, vector, _, _) =>
        val prediction = model.predict(vector, threshold)
