...
/**
  ...
  * the same except for numerical errors.
  *
...
class NormalizationIntegTest extends SparkTestUtils {
  ...
  private def generateRandomModel(seed: Int): LogisticRegressionModel = {
    ...
    val coef = (for (_ <- 0 to _dimension) yield Random.nextGaussian()).toArray
  ...
  private def generateSampleRDD(seed: Int, model: LogisticRegressionModel): RDD[LabeledPoint] = {
    val data =
      drawBalancedSampleFromNumericallyBenignDenseFeaturesForBinaryClassifierLocal(seed, _size, _dimension).map {
  ...
  /**
    * Check the correctness of training with a specific normalization type. This check involves an unregularized
    * training. The prediction of an unregularized trained model should predictClass exactly the same label as
    * the one in the input training data, given that the labels are generated by a model without noise.
  ...
  private def checkTrainingOfNormalizationType(trainRDD: RDD[LabeledPoint], testRDD: RDD[LabeledPoint],
    ...
    assertTrue(correct.toDouble / _size >= _precision,
      s"Precision check [$correct/${_size} >= ${_precision}] failed.")
  }
}
