...
class AvroDataReaderTest extends SparkTestUtils {
  ...
  def testRead(): Unit = sparkTest("testRead") {
    ...
    val (df, _) = dr.readMerged(inputPath, featureSectionMap)
  ...
  def testReadWithFeatureIndex(): Unit = sparkTest("testReadWithIndex") {
    ...
    val dr = new AvroDataReader(sc)
    val df = dr.readMerged(inputPath, indexMapLoaders, featureSectionMap)
  ...
  def testReadMultipleFiles(): Unit = sparkTest("testReadMultipleFiles") {
    val dr = new AvroDataReader(sc)
    val (df, _) = dr.readMerged(Seq(inputPath, inputPath2), featureSectionMap)
  ...
  def testReadDuplicateFeatures: Unit = sparkTest("testReadDuplicateFeatures") {
    ...
    try {
      val (df, _) = dr.read(duplicateFeaturesPath)
