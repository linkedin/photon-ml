...
class BootstrapTrainingDiagnostic(private val unparsedNameTermToIndex: Map[String, Int],
  ...
  override def diagnose(modelFactory: (RDD[LabeledPoint], Map[Double, GeneralizedLinearModel]) => List[(Double, GeneralizedLinearModel)],
    ...
    val aggregates = BootstrapTraining.bootstrap[GeneralizedLinearModel](numSamples, trainingPortion, models, modelFactory, aggregators, trainingData)

    // lambda -> metric -> metric distribution
    val metrics = aggregates.mapValues(aggregate => {
      aggregate.get(METRICS_AGGREGATION) match {
        // Should never see another case. If we do, we changed things BootstrapTraining.aggregateMetricsConfidenceIntervals and so
        // the match failure exception that we see should be a clue that this here also needs to be updated.
    ...
    metrics.map(item => {
      ...
      val coefficientSummary = coefficients.get(lambda).get // should always have the same lambdas as keys in both these containers.
      val numStraddlingZero = coefficientSummary._1.filter(x => x.estimateFirstQuartile() < 0 && x.estimateThirdQuartile() > 0).size
      val m = metricSummary.mapValues(x => (x.getMin(), x.estimateFirstQuartile(), x.estimateMedian(), x.estimateThirdQuartile(), x.getMax()))
      val c = coefficientSummary._1.map(x => (x.getMin(), x.estimateFirstQuartile(), x.estimateMedian(), x.estimateThirdQuartile(), x.getMax()))

      val straddlingZero = importances.getOrElse(lambda, Map.empty[(String, String), (Int, Double, CoefficientSummary)])
        .toSeq
        .filter(x => x._2._3.estimateFirstQuartile < 0 && x._2._3.estimateThirdQuartile > 0)
        .sortBy(x => x._2._2).toMap

      val importantFeatures = importances.get(lambda).get.toSeq.sortBy(_._2._2).takeRight(NUM_IMPORTANT_FEATURES).map(x => (x._1, x._2._3)).toMap
