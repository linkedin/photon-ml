...
case class LogisticRegressionOptimizationProblem(
  ...
  extends GeneralizedLinearOptimizationProblem[LogisticRegressionModel, TwiceDiffFunction[LabeledPoint]](
    optimizer,
    objectiveFunction,
    sampler,
  ...
  /**
   * Create a default logistic regression model with 0-valued coefficients
   *
  ...
  override protected[optimization] def createModel(
      ...
      variances: Option[Vector[Double]]): LogisticRegressionModel =
    new LogisticRegressionModel(Coefficients(coefficients, variances))
