...
/**
 * Train a classification model for Logistic Regression using L1/L2/Elastic net-regularized linear regression.
 *
...
class LogisticRegressionAlgorithm
  ...
  override protected def createModel(coefficients: Vector[Double]) = {
    new LogisticRegressionModel(Coefficients(coefficients, variancesOption = None))
  }
}
