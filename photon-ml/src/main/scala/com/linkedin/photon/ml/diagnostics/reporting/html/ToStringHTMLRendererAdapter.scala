 ...
 */
class ToStringHTMLRendererAdapter[-P <: PhysicalReport : ClassTag](namespaceMap: NamespaceBinding, htmlPrefix: String, svgPrefix: String) extends SpecificRenderer[P, Node] {
  ...
  def render(p: P): Node = {
    wrappedRenderer.render(
      new SimpleTextPhysicalReport(
        s"Do not know how to directly render instances of [${p.getClass.getName}] to HTML. Stringified version follows.\n$p"
      )
    )
  }
