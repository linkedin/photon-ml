...
class ModelDataScoresTest extends SparkTestUtils {
   ...
   * @return The [[ModelDataScores]] generated with the given keys and values
   */
  private def generateFullDataScores(keys: Array[Long], values: Array[Double]): ModelDataScores =
    new ModelDataScores(sc.parallelize(keys.zip(values.map(v => ScoredGameDatum(score = v)))))

  @Test
  def testEquals(): Unit = sparkTest("testEqualsForKeyValueScores") {
    //case 1: key value scores of length 0
    val emptyScores1 = generateFullDataScores(scores = Array[Double]())
    val emptyScores2 = generateFullDataScores(scores = Array[Double]())
    assertEquals(emptyScores1, emptyScores2)

    //case 2: key value scores with different length
    val nonEmptyScores1 = generateFullDataScores(scores = Array[Double](1, 2))
    val nonEmptyScores2 = generateFullDataScores(scores = Array[Double](1, 2, 3))
    ...
    val keyValueScore1 = generateFullDataScores(sharedKeys, values1)
    val keyValueScore2 = generateFullDataScores(sharedKeys, values2)
    ...
    val keyValueScore3 = generateFullDataScores(keys1, sharedValues)
    val keyValueScore4 = generateFullDataScores(keys2, sharedValues)
    ...
    val keyValueScore5 = generateFullDataScores(keys3, values3)
    val keyValueScore6 = generateFullDataScores(keys4, values4)
  ...
  def testPlus(): Unit = sparkTest("testPlusForKeyValueScores") {
    ...
    val emptyScores = generateFullDataScores(scores = Array[Double]())
    assertEquals(emptyScores - emptyScores, emptyScores)

    //case 2: one of the scores are empty
    val nonEmptyScores = generateFullDataScores(scores = Array[Double](1, 2))
    ...
    val keyValueScore1 = generateFullDataScores(keys1, values1)
    val keyValueScore2 = generateFullDataScores(keys2, values2)
  ...
  def testMinus(): Unit = sparkTest("testMinusForKeyValueScores") {
    ...
    val expectedNonEmptyScores = generateFullDataScores(scores = Array[Double](-1, -2))
