...
trait L2Regularization extends ObjectiveFunction {
  ...
  protected var interceptIndexOpt: Option[Int] = None

  require(interceptIndexOpt.isEmpty || interceptIndexOpt.forall(_ >= 0), "Intercept index is negative.")
  ...
  protected def l2RegValue(coefficients: Vector[Double]): Double = {
    ...
    val unweightedL2Regularization = interceptIndexOpt match {
      case None => coefSquaredMagnitude
